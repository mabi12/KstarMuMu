...
B_mass_frac = RooRealVar("frac", "frac", 0.5, 0.01, 1)
bg_exp_lambda_cb = RooRealVar("exp_lambda", "exp_lambda",  -0.01, -1, 0)
bg_exp_cb= RooExponential("bg_exp", "bg_expo", m_B, bg_exp_lambda_cb)
cb = RooCBShape("CrystalBall", "Crystal Ball", m_B, cb_mean, cb_sigma, cb_alpha, cb_n)
bg_sg_exp_cb = RooAddPdf("bg_sg_exp_cb", "Superposition of Exp and Crystal Ball (bg + sg)", RooArgList(cb, bg_exp_cb), RooArgList(B_mass_frac), True)
B_mass_fit['cb'] = fitFunction(cb, RooArgList(cb_mean, cb_sigma, cb_alpha, cb_n, bg_exp_lambda_cb, B_mass_frac))
...
g2_sigma = RooRealVar("B_mass_g2_sigma", "Sigma 2", 100, 0, 135)
...
dg = RooAddPdf("DoubleGaussian","Double Gaussian",RooArgList(g1,g2),RooArgList(B_mass_frac),True)
#bg_sg_dg = RooAddPdf("bg_sg_Doublegaussian", "Superposition of Double Gaussian and Exp (bg + sg)")
B_mass_fit['dg'] = fitFunction(dg, RooArgList(g1_mean, g1_sigma, g2_mean, g2_sigma, B_mass_frac))
...
sg_exp = RooExponential("expo", "expo", m_B, sg_exp_lambda)
bg_exp_lambda_gaussian_2 = RooRealVar("exp_lambda", "exp_lambda",  -0.01, -1, 0)
bg_exp_gaussian_2 = RooExponential("expo", "expo", m_B, bg_exp_lambda_gaussian_2)
superposed_exp_gaussian = RooAddPdf("g3_exp", "Superposition of Exp and Gaussian", RooArgList(g_3, sg_exp), RooArgList(B_mass_frac), True)
bg_sg_superposed_exp_gaussian = RooAddPdf("bg_sg_g3_exp", "Superposition of Exp and Exp + Gaussian (bg + sg)", RooArgList(superposed_exp_gaussian, bg_exp_gaussian_2,), RooArgList(B_mass_frac), True)
B_mass_fit["expg"] = fitFunction(bg_sg_superposed_exp_gaussian, RooArgList(g_mean_3, g_sigma_3, sg_exp_lambda, bg_exp_lambda_gaussian_2, B_mass_frac))

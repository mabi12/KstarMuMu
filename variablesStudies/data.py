...
ntuple =  "data18_13TeV_periodK_part_03"
...
range_ = {"BmumuKst_B_mass" : "BmumuKst_B_mass>5150 && BmumuKst_B_mass<5700",
          ...
          "BmumuKst_diMuon_mass*BmumuKst_diMuon_mass" : "BmumuKst_diMuon_mass*BmumuKst_diMuon_mass>1000000 &&  BmumuKst_diMuon_mass*BmumuKst_diMuon_mass < 8000000",
...
file_name = os.path.join(path,"ntuple-"+ntuple+".root")
...
if draw:
    for j,v in enumerate(tqdm(range_)):
        ...
        c.cd(3+3*j)
        t.Draw(v,range_[v]+" && "+cuts_run1)
        rename_htemp("RUN 1 cuts")    

    c.Draw()
    c.SaveAs(f"out/{ntuple}.png")
    c.SaveAs(f"out/{ntuple}.pdf")
